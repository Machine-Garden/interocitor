...
The comparisons below are about that boundary. On the device the posture is the
opposite: every key-bearing endpoint is fully trusted, and the local row store
...
| Signal               | The mailbox stores and returns encrypted artifacts. With a mesh key it never receives message plaintext, and object names are keyed hashes.                                                                                                                                  | Less secure than Signal: private chat is more than encryption, and there is no ratchet, no sealed sender, no safety numbers, and no key rotation at all — rotating means creating a new mesh — so a copied key opens the whole history. Signal also encrypts its local database under an OS-keychain key; the local rows here are not encrypted at rest. Ahead of XChat at the remote boundary: nothing the operator holds is readable without the mesh key. |
| Cryptomator          | Files are encrypted before the WebDAV, Google Drive, or iCloud folder adapter sees them, on storage the user already pays for and owns.                                                                                                                                      | Equal at the remote boundary, since both encrypt before the folder adapter sees a byte and both leave sizes visible. Opposite on the device: Cryptomator's product is an encrypted local vault, while the local row store here is not encrypted at rest. Behind Cryptomator as a drop-in virtual drive; ahead when the files belong to an application with rows that need to merge.                                                                          |
| Bitwarden            | A portable key, device pairing, and recovery phrases are built in, so a vault syncs across devices without a custodial server.                                                                                                                                               | Bitwarden's server also never sees the vault, and its client vault is encrypted at rest under a master-password KDF and locks when idle; the default here is a local store that is plaintext at rest with no lock concept. It offers emergency access and organization administration that Interocitor leaves to the host to build; in exchange there is no custodial account or server at all.                                                              |

        ...
        <section id="why" className="site-section why-section" aria-labelledby="why-title">
          ...
          <div className="shell build-list ladder-list" aria-labelledby="ladder-title">
            <div className="build-list-heading">
              ...
              <p>
                Three packages, one engine. Each rung adds a runtime concern on top of the one below
                it and never replaces the API underneath. Stop at the rung your runtime needs.
              </p>
            </div>
            <dl>
              <div>
                ...
                <dd>
                  The runtime-neutral engine: rows, sync, encryption, pairing, recovery, durable
                  files, and the memory, WebDAV, Google Drive, and Cloudflare adapters. Stop here
                  for a Node worker, a script, an agent, or a test that brings its own local store.
                </dd>
              </div>
              <div>
                <dt>2 · @interocitor/web</dt>
                <dd>
                  What a browser wires into that engine: IndexedDB local stores, a credential
                  custody choice from tab session to passkey, image helpers, and local reset. Stop
                  here for a browser app without React.
                </dd>
              </div>
              <div>
                <dt>3 · @interocitor/react</dt>
                <dd>
                  Bindings only: typed context, live queries and rows, image URLs, connection
                  status, and connected-store views. The app still builds and connects the engine;
                  mutations need no wrapper.
                </dd>
              </div>
            </dl>
            <p className="build-list-note">
              Python and Swift implement the same protocol at rung one and join the same meshes. The
              Cloudflare worker is a mailbox, not a rung.{" "}
              <SpaLink href="integrations.html">Read the ladder in full.</SpaLink>
            </p>
          </div>
        </section>
            ...
            <nav className="decision-chooser-grid" aria-label="Planning guides">
              ...
              <SpaLink className="decision-choice" href="integrations.html">
                ...
                <div>
                  ...
                  <p>Climb the ladder from core to web to React and stop at your runtime.</p>
                </div>
                <span aria-hidden="true">↗</span>
              </SpaLink>
            </nav>

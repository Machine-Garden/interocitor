...
test("the established landing page renders with deployment security headers", async ({
  page,
}) => {
  ...
  await expect(page).toHaveTitle(
    "Interocitor — local-first app data without trusting the cloud",
  );
  await expect(
    page.locator(
  ...
  await expect(
    page.getByRole("heading", { name: "Your app keeps working." }),
  ).toBeVisible();
  await expect(
    page.locator('.site-hero a[href="/examples/todomvc/"]'),
  ).toBeVisible();
  await expect(
    page.locator('.task-example a[href="/examples/board/"]'),
  ).toBeVisible();
  await expect(
    page.locator('.task-example a[href="/examples/family-locator/"]'),
  ).toBeVisible();
  await expect(page.locator(".site-hero .actions a")).toHaveCount(2);
  await expect(page.locator(".decision-choice")).toHaveCount(6);
  await expect(page.locator('.decision-choice[href="/auth"]')).toBeVisible();
  await expect(
    page.locator('.remotes-summary a[href="/storage"]'),
  ).toContainText("See how Interocitor stores data");
  await expect(page.locator('.nav-cta[href="/how-it-works"]')).toContainText(
    "Docs",
  );
});

test("Markdown documentation owns its content, outline, and metadata", async ({
  page,
}) => {
  const documentation = new Map([
    ...
    [
      "/data-boundaries",
      "What belongs in Interocitor rows, files, and meshes?",
    ],
  ...
  for (const [route, title] of documentation) {
    ...
    await expect(
      page.locator(
        '.docs-content a[href*="github.com/Machine-Garden/interocitor/blob/main"]',
      ),
    ).toHaveCount(0);
  }
  ...
  await expect(
    page.locator('#storage + p + p a[href="/storage"]'),
  ).toBeVisible();
  ...
  for (const profile of [
    ...
    await expect(
      page.getByRole("heading", { level: 3, name: profile, exact: true }),
    ).toBeVisible();
  }
...
test("planning links use client-side navigation", async ({ page }) => {
  ...
  await expect(page.locator(".docs-hero h1")).toContainText(
    "Four locks guard one mesh",
  );
  expect(documentRequests).toBe(0);
});

test("the auth guide keeps independent access capabilities separate", async ({
  page,
}) => {
  ...
  await expect(page.locator("#recovery")).toContainText(
    "Twelve random BIP-39 words",
  );
  await expect(page.locator("#taints")).toContainText("A taint is not an ACL");
  await expect(page.locator("#grants")).toContainText(
    "Authentication still stays outside",
  );
});

test("flow charts render as diagrams instead of source code", async ({
  page,
}) => {
...
test("documentation starts within the first desktop viewport", async ({
  page,
}) => {
...
test("documentation guides lead with their subject and close at the intended boundary", async ({
  ...
  const guides = new Map([
    ...
    ["/applications", "Start from the guarantees"],
    ["/integrations", "Climb the ladder one rung at a time"],
  ...
  for (const [route, opening] of guides) {
    ...
    await expect(headings.last(), route).toHaveText(
      route === "/storage"
        ? "Annex: Cloudflare Free numbers"
        : "Decision summary",
    );
  }
});

test("short documentation routes keep their public targets", async ({
  request,
}) => {
  const routes = new Map([
    ...
    [
      "/core",
      "https://github.com/Machine-Garden/interocitor/tree/main/packages/core#readme",
    ],
    [
      "/web",
      "https://github.com/Machine-Garden/interocitor/tree/main/packages/web#readme",
    ],
    [
      "/react",
    ...
    [
      "/examples",
      "https://github.com/Machine-Garden/interocitor/tree/main/examples",
    ],
  ]);
...
test("old deep-doc shortcuts now lead back to the public guides", async ({
  request,
}) => {
  const routes = new Map([
...
test("landing and documentation stay usable at a narrow viewport", async ({
  page,
}) => {
  ...
  await expect(page.locator("#decisions h2")).toContainText(
    "Plan the system boundaries",
  );
  expect(
...
test("live examples remain reachable from the SPA", async ({ page }) => {
  ...
  for (const route of routes) {
    ...
    await expect(example.locator("code"), route).toContainText(
      "const schema = {",
    );

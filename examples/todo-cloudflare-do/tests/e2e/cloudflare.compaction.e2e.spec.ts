...
test("Cloudflare TODO demo rehydrates a fresh tab from the compacted mainline snapshot", async ({
  browser,
  baseURL,
}) => {
  const {
  ...
  try {
    ...
    await expect
      .poll(async () => await getTitles(reader), {
        timeout: 5_000,
        message:
          "Expected a fresh Cloudflare demo tab to see compacted tasks after rehydrating from mainline.",
      })

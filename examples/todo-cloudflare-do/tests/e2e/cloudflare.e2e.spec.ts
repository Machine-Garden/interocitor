...
test("Cloudflare TODO demo syncs across tabs under prefixed worker routes", async ({
  browser,
  baseURL,
}) => {
  const {
  ...
  try {
    ...
    await expect
      .poll(async () => await getTitles(tabB), {
        timeout: 15_000,
        intervals: [250, 500, 1000],
        message:
          "Expected tab B to receive the new task through the Cloudflare worker-backed adapter.",
      })
      .toContain(title);
  } finally {
    await context.close();

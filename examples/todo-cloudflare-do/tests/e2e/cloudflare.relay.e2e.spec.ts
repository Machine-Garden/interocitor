...
test("request budget: Cloudflare reconnect does not rewrite device metadata", async ({
  browser,
  baseURL,
}) => {
  const {
    context,
    pages: [page],
  } = await newDemoPages(browser, baseURL!, 1);
...
test("request budget: Cloudflare invalidation delivery stays bounded across two tabs", async ({
  browser,
  baseURL,
}) => {
  const {
    context,
    pages: [tabA, tabB],
  } = await newDemoPages(browser, baseURL!, 2);
...
test("request budget: Cloudflare device metadata writes do not wake other tabs", async ({
  browser,
  baseURL,
}) => {
  const {
  ...
  try {
    ...
    const response = await tabA.evaluate(async (currentSession) => {
      ...
      const encodedPath = encodeURIComponent(
        `${currentSession.remotePath}/devices/manual-heartbeat.json`,
      );
      const res = await fetch(
        `${currentSession.workerBaseUrl}/io/${encodedNamespace}/file?path=${encodedPath}`,
        {
          method: "PUT",
          headers: {
...
test("request budget: Cloudflare relay-disabled client falls back to polling without websocket invalidations", async ({
  browser,
  baseURL,
}) => {
  const {
  ...
  try {
    ...
    expect(
      (statsB.fetch["GET file:head"] ?? 0) + (statsB.fetch["POST list-files"] ?? 0),
    ).toBeGreaterThan(0);
  } finally {
    await context.close();
  }
...
test("Cloudflare TODO worker exposes InterocitorRelayDurableObject on /notify/<address>", async ({
  browser,
  baseURL,
}) => {
  const {
  ...
  try {
    ...
    const outcome = await page.evaluate(
      async ({ notifyUrl: browserNotifyUrl, writeUrl: browserWriteUrl }) => {
        return await new Promise<string>((resolve) => {
          ...
          ws.onopen = () => {
            void fetch(browserWriteUrl, { method: "PUT", body: "relay-proof" }).catch(() =>
              resolve("write-error"),
            );
          };
          ws.onmessage = (event) => {

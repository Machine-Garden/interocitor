...
export const CF_MESH_BEARER_SECRET =
  process.env.PLAYWRIGHT_CF_MESH_BEARER_SECRET || "playwright-access-secret";
export const CF_SYSTEM_BEARER_TOKEN =
  process.env.PLAYWRIGHT_CF_SYSTEM_BEARER_TOKEN || "playwright-system-secret";
export const CF_MESH_SECRET =
  process.env.PLAYWRIGHT_CF_MESH_SECRET || "replace-with-production-mesh-secret";
...
export async function applySession(
  ...
  relayEnabled = true,
  relayHealthyPollInterval = 300_000,
): Promise<void> {
  await page.evaluate(
    ({
      raw,
      ...
      relayHealthyPollInterval: nextRelayHealthyPollInterval,
    }) => {
      window.__todoDemo.configure({
        pollInterval: nextPollInterval,
        relayEnabled: nextRelayEnabled,
        relayHealthyPollInterval: nextRelayHealthyPollInterval,
      });
...
export async function newDemoPages(
  browser: Browser,
  baseURL: string,
  count: number,
): Promise<{ context: BrowserContext; pages: Page[] }> {
  ...
  interface Window {
    __todoDemo: {
      ...
      configure(options: {
        ...
        relayEnabled?: boolean;
        relayHealthyPollInterval?: number;
      }): { pollInterval: number; relayEnabled: boolean; relayHealthyPollInterval: number };
      getShareToken(): string;
      getStatus(): string;
      resetRequestStats(): void;
      getRequestStats(): {
        fetch: Record<string, number>;
        websocket: { opened: number; messages: number; closed: number; errors: number };
      };
    };
  }
}

...
test("TODO demo shares join token across tabs and syncs over local WebDAV", async ({
  browser,
  baseURL,
}) => {
...
test("local WebDAV server supports multiple remote paths without cross-talk", async ({
  browser,
  baseURL,
}) => {
  const context = await browser.newContext();

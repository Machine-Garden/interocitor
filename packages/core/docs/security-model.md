...
The README has a one‑paragraph summary. This file is the contract.

## What encryption protects

When `encrypted: true` (the default) the engine uses **AES‑GCM with a
256‑bit key**. The key is derived deterministically from a base58
...
- **Forward secrecy across keys.** Reading a passphrase issued today does
  not let you decrypt files written under a previous passphrase. (The
  engine has no in‑place re‑encryption tool — see "Schema migration" in
  the README for the recommended dump‑and‑restore.)
...
- **Passphrase strength.** A weak passphrase means a weak key. The engine
  ...
  KDF stretching** (no PBKDF2, no Argon2). A passphrase generated by
  `generateKey()` is 32 random bytes encoded as base58 and is safe. A
  human‑chosen short passphrase is **not safe**. Always generate.
...
- **Credential store at rest.** The default
  `LocalStorageCredentialStore` writes the passphrase to `localStorage`
  in plain text. Use the `WebAuthnCredentialStore` (largeBlob backed by
  the OS keychain, gated by Touch ID / Face ID) for stronger device‑side
  protection. See `docs/credential-store.md`.
...
- **Device identity** is a client‑generated UUIDv7 stored in
  `localStorage["interocitor-device-id"]`. There is no remote registration
  step. Anyone with write access to the remote can claim any device id.
...
- **Passphrase = capability.** Knowing the passphrase is sufficient to
  read and write the mesh. Loss of the passphrase = loss of the mesh
  (see "Restore" in the README). Theft of the passphrase = silent
  compromise; rotate by creating a new mesh and migrating data out
  of the old one.
...
| Goal | Setting |
| --- | --- |
| Protect row contents from the storage operator | `encrypted: true` (default) |
| Generate a strong key | Let the engine generate; expose via `getPassphrase()` |
| Resist passphrase exfiltration on the device | Use `WebAuthnCredentialStore` (Touch ID / Face ID, OS keychain) |
...
- Per‑device revocation. Removing a device from a mesh requires rotating
  the passphrase, which means creating a new mesh.

...
/** Resolve user input or a legacy manifest to the finite retention contract. */
export function resolveRetentionPolicy(input?: RetentionPolicyInput | null): RetentionPolicy {
  return {
    compactAfterMs: finitePositiveDuration('retention.compactAfterMs', input?.compactAfterMs ?? DEFAULT_COMPACT_AFTER_MS),

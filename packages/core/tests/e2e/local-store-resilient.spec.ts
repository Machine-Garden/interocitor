...
test.describe('Resilient LocalStore', () => {
  ...
  test('createResilientLocalStore falls back to memory when primary open() hangs past deadline', async ({ page }) => {
    const result = await page.evaluate(async () => {
  ...
  test('createResilientLocalStore uses primary normally when it opens promptly', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { createResilientLocalStore } = await import('/packages/core/dist/storage/resilient-store.js');
      const { MemoryLocalStore } = await import('/packages/core/dist/storage/memory-store.js');
  ...
  test('createResilientLocalStore does NOT fall back when primary is slow but reports progress', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const slowButProgressingPrimary = {
        ...
        open: (onProgress?: () => void) => new Promise<void>(resolve => {
  ...
  test('createResilientLocalStore falls back when primary open() throws synchronously', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { createResilientLocalStore } = await import('/packages/core/dist/storage/resilient-store.js');
      ...
      const origError = console.error;
      console.error = () => { /* swallow for cleaner test output */ };
      try {
        const store = createResilientLocalStore({
          openTimeoutMs: 1000,

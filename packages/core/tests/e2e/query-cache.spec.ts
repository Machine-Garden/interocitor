  ...
  test('row cache: lazy build, then-driven load, dedupe, stale-while-revalidate, deletion', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        readyTitle: (ready.row as any)?.title?.value ?? null,
        statusDuring: duringRevalidate.status,
        duringTitle: (duringRevalidate.row as any)?.title?.value ?? null,
        afterRevalidateTitle: (afterRevalidate.row as any)?.title?.value ?? null,

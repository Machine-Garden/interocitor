...
test.describe("Interocitor query cache", () => {
  ...
  test("mutation triggers stale-while-revalidate: old rows visible while pending, then refresh", async ({
    page,
  }) => {
    const result = await page.evaluate(async () => {
      ...
      class DeferredTableReadStore extends MemoryLocalStore {
        private deferredRead: Promise<void> | null = null;
        private releaseRead: (() => void) | null = null;

        deferNextTableRead(): void {
          this.deferredRead = new Promise((resolve) => {
        ...
        override async getTable(table: string) {
      ...
      const localStore = new DeferredTableReadStore();
      ...
      localStore.releaseDeferredRead();
  ...
  test("remote invalidation refreshes a ready empty query cache entry", async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const writer = new Interocitor(adapter, {
        remotePath: "/QC-remote-invalidation",
        pollInterval: 600_000,
        deviceId: "writer",
        keySource: null,
      ...
      return {
        ...
        events,
        cacheTitlesAfter: (afterCache.rows ?? []).map(
          (row: any) => row.payload?.title?.value ?? null,
        ),
  ...
  test("row cache: lazy build, then-driven load, dedupe, stale-while-revalidate, deletion", async ({
    page,
  }) => {
    const result = await page.evaluate(async () => {
      const { BrowserTestInterocitor: Interocitor, MemoryLocalStore } =
      ...
      class DeferredInvalidationRowStore extends MemoryLocalStore {
        private armAfterCommit = false;
        private blockNextRowRead = false;
        private deferredRead: Promise<void> | null = null;
        private releaseRead: (() => void) | null = null;

        deferInvalidationReadAfterNextCommit(): void {
          this.armAfterCommit = true;
          this.deferredRead = new Promise((resolve) => {
            this.releaseRead = resolve;
          });
        }
        ...
        override async getRow(table: string, rowId: string) {
          if (this.blockNextRowRead) {
            this.blockNextRowRead = false;
            await this.deferredRead;
            this.deferredRead = null;
          }
          return super.getRow(table, rowId);
        }
      }

      const localStore = new DeferredInvalidationRowStore();
      ...
      localStore.deferInvalidationReadAfterNextCommit();
      await engine.put("tasks", "t1", { title: "b" });
      ...
      const afterRevalidateTitle = (afterRevalidate.row as any)?.payload?.title?.value ?? null;

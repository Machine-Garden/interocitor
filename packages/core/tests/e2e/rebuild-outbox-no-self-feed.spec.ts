 ...
 *   adapter auth, the reload should only probe remote head and then do
 *   nothing else:
 *     - no remote writes
 *     - no listFiles
...
test('reload after local writes reads head and performs no extra remote activity', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const createCountingRemote = (inner: InstanceType<typeof MemoryAdapter>) => new Proxy(inner, {
    ...
    const baseConfig = {
      ...
      autoCompact: false,
      batchWindowMs: 0,
      localStoreFactory: () => new CountingLocalStore('rebuild-outbox-no-self-feed'),
    } as const;

    // First session: write a few legitimate changes and flush them once.
    const first = new Interocitor(firstRemote as any, baseConfig);
    await first.init();
    await first.connect();
    ...
    const reload = new Interocitor(reloadRemote as any, baseConfig);
    await reload.init();
    ...
    const remoteDelta = Object.fromEntries(
      Object.keys(remoteCalls).map(k => [k, afterReloadRemote[k as keyof typeof remoteCalls] - beforeReloadRemote[k as keyof typeof remoteCalls]]),
    ) as typeof remoteCalls;
    const localDelta = Object.fromEntries(
      Object.keys(localCalls).map(k => [k, afterReloadLocal[k as keyof typeof localCalls] - beforeReloadLocal[k as keyof typeof localCalls]]),
  ...
  // And then nothing else remote-side.
  expect(result.remoteDelta.ensureFolder).toBe(0);
  expect(result.remoteDelta.listFiles).toBe(0);

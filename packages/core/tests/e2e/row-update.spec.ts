  ...
  test('patch overwrites only listed fields; absent fields untouched', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U1', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
...
test.describe('Table.replace — full overwrite', () => {
  test('nulls fields absent from data', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const { MemoryAdapter } = await import('/packages/core/dist/adapters/memory.js');
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U2', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
      await engine.connect();
      const tasks = engine.table('tasks');
  ...
  test('never touches engine _meta (table/rowId/deleted) — meta is a separate namespace', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const { Interocitor } = await import('/packages/core/dist/index.js');
      const { MemoryAdapter } = await import('/packages/core/dist/adapters/memory.js');
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U3', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
      await engine.connect();
      const tasks = engine.table('tasks');
  ...
  test('tolerates user payload keys named _table, _rowId, _meta, payload — meta isolation', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const { MemoryAdapter } = await import('/packages/core/dist/adapters/memory.js');
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U4', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
      await engine.connect();
      const weeks = engine.table('weekPlans');
      // Migrating from legacy data — item still has _table, _rowId fields.
      await weeks.replace('w1', { _table: 'oldname', _rowId: 'w1', _meta: { foo: 1 }, payload: { x: 1 }, weekId: 'w1', plan: { mon: ['x'] } } as any);
...
test.describe('HLC monotonicity', () => {
  test('rewriting same field bumps the column HLC strictly forward', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const { MemoryAdapter } = await import('/packages/core/dist/adapters/memory.js');
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U5', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
      await engine.connect();
      const tasks = engine.table('tasks');
...
test.describe('Tombstone resurrection', () => {
  test('delete then put restores the row with deleted=false', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const { MemoryAdapter } = await import('/packages/core/dist/adapters/memory.js');
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U6', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
      await engine.connect();
      const tasks = engine.table('tasks');
      ...
      return {
        ...
        afterRevive: afterRevive ? { deleted: afterRevive._meta.deleted, title: afterRevive.payload.title.value } : null,
      };
    });
...
test.describe('Outbox accounting', () => {
  test('one op per patch, one op per delete', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const local = new LocalStore('outbox-count');
      const engine = new Interocitor(new MemoryAdapter(), {
...
test.describe('Row cache invalidation', () => {
  test('patch invalidates the row cache, replace too', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      const { MemoryAdapter } = await import('/packages/core/dist/adapters/memory.js');
      const engine = new Interocitor(new MemoryAdapter(), {
        remotePath: '/U8', pollInterval: 600_000, deviceId: 'dev_a',
      });
      await engine.init();
      await engine.connect();
      const tasks = engine.table('tasks');

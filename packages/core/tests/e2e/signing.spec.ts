...
test.describe('asymmetric signing', () => {
  test('sign/verify raw bytes and round-trip exported keys', async ({ page }) => {
    const result = await page.evaluate(async () => {
      ...
      // Tampered data fails.
      const tampered = await verify(pub, new TextEncoder().encode('attest this payloaX'), signature);
  ...
  test('signToken/verifyToken carry claims, reject tampering, and honor expiry', async ({ page }) => {
    const result = await page.evaluate(async () => {
      const {
        generateSigningKeypair,
        exportPublicKey,
      ...
      const token = await signToken(privateKey, { sub: 'device-1', scope: 'read' }, { expiresInSeconds: 60 });
      ...
      const expired = await signToken(privateKey, { sub: 'device-1' }, { issuedAt: 1000, expiresInSeconds: 1 });

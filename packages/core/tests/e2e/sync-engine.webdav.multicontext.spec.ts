...
test("two isolated contexts sync via shared WebDAV route mock", async ({ browser, baseURL }) => {
  ...
  try {
    ...
    await pageA.goto(`${baseURL}/packages/core/tests/e2e/fixtures/harness-plain.html`);
    await pageB.goto(`${baseURL}/packages/core/tests/e2e/fixtures/harness-plain.html`);
...
test("isolated contexts can detach, switch WebDAV backends, and later rejoin the old backend", async ({
  browser,
  baseURL,
}) => {
  ...
  try {
    ...
    expect(clientOneAfterRejoin).toEqual([
      "from first while detached",
      "from old adapter",
      "seed offline",
    ]);
    expect(clientTwoAfterPull).toEqual([
      "from first while detached",
      "from old adapter",
      "seed offline",
    ]);

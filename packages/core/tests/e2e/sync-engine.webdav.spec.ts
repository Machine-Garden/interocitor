...
test("request budget: WebDAV reconnect avoids device rewrite churn", async ({ page }) => {
  ...
  expect(result.counts["GET /InterocitorBudget/devices/dev_budget.json"] ?? 0).toBeLessThanOrEqual(
    1,
  );
});
...
test("request budget: WebDAV repeated pull with no changes does not write", async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const engine = new Interocitor(
      ...
      {
        deviceId: "dev_pull_budget",
        remotePath: "/InterocitorPullBudget",
        pollInterval: 60_000,
        flushDebounce: 5,
        flushThreshold: 1,
      },
  ...
  const writeKeys = Object.keys(result).filter(
    (key) => key.startsWith("PUT ") || key.startsWith("DELETE ") || key.startsWith("MKCOL "),
  );
...
test("Interocitor writes file-per-change paths and syncs rows through WebDAV", async ({ page }) => {
  ...
  expect(result.cloudFiles.some((path) => /\/changes\/[^/]+-chg_[^/]+\.json$/.test(path))).toBe(
    true,
  );
...
test("encrypted sync over WebDAV keeps cloud payload opaque", async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const cloud = window.__webdavMock.dumpFiles();
    const payload = Object.entries(cloud).find(([path]) =>
...
test("direct-cloud compaction over WebDAV restores clients from snapshot", async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    return {
      ...
      hasSnapshot: Object.keys(window.__webdavMock.dumpFiles()).some((path) =>
        path.includes("/WebCompact/mainline/snapshot-1-"),
      ),
    };
  });

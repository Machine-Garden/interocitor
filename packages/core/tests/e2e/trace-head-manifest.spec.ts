 ...
 *  2. Steady-state flush: the current remote retention policy is refreshed,
 *     then the publication path uses the resulting manifest cache entry.
...
test("trace events explain head + manifest IO and prove no redundant rewrites", async ({
  page,
}) => {
  ...
  expect(bootstrapWrites.map((w) => w.reason).toSorted(), "two writes: manifest + pointer").toEqual(
    ["bootstrap", "bootstrap-pointer"],
  );
  expect(
  ...
  expect(
    f1Manifest.filter((m) => m.op === "read" && m.reason === "flush-retention"),
    "flush refreshes the authoritative retention policy",
  ).toHaveLength(2);
  expect(f1Manifest.filter((m) => m.op === "read" && m.reason === "flush")).toHaveLength(0);
  expect(
    f1Manifest.filter((m) => m.op === "cache-hit" && m.reason === "flush"),
    "publication reuses the refreshed manifest without another lookup",
  ).toHaveLength(0);
  ...
  expect(f2Manifest.filter((m) => m.op === "read" && m.reason === "flush-retention")).toHaveLength(
    2,
  );
  expect(f2Manifest.filter((m) => m.op === "read" && m.reason === "flush")).toHaveLength(0);
  expect(f2Manifest.filter((m) => m.op === "cache-hit" && m.reason === "flush")).toHaveLength(0);
  ...
  for (const ev of [...result.flush1, ...result.flush2]) {
    if (ev.type === "trace:head" && ev.op === "write") {
      expect(
        ev.regressed,
        `head must never regress (prior=${ev.priorHlc} next=${ev.nextHlc})`,
      ).toBe(false);
    }
  }
});

test("trace head: future-prior HLC short-circuits via skip-no-change (no regression write)", async ({
  page,
}) => {
  const result = await page.evaluate(async () => {
    const { BrowserTestInterocitor: Interocitor } =
    ...
    engine.on((e: any) => {
      if (e.type === "trace:head") headEvents.push({ ...e });
    });
  ...
  expect(result.headOnDisk.latestHlc, "head on disk preserved at future HLC").toBe(
    result.futureHlc,
  );
});

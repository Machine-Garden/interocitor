...
test("MemoryLocalStore contract: local mutation staging and exact acknowledgement are crash-safe", async () => {
  await withStore(async (store) => {
    ...
    assert.deepEqual(
      (await store.peekOutbox()).map((entry) => entry.id),
      ["chg_3"],
    );
  });
});
...
test("MemoryLocalStore contract: independent writers merge the durable pending batch", async () => {
  await withStore(async (store) => {
    const first = {
      ...
      device: "dev",
      hlc: "h1",
      ops: [{ type: "delete", table: "tasks", rowId: "a", hlc: "h1" }],
    };
    const second = {
      id: "chg_second",
      ts: 2,
      device: "dev",
      hlc: "h2",
      ops: [{ type: "delete", table: "tasks", rowId: "b", hlc: "h2" }],
    };

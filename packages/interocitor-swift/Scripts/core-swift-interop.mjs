...
function assertMixedRuntimeRows(rows) {
  ...
  assert(swiftRow?.title === 'Created by InterocitorSwift', `Swift-created row is absent or changed; received ${received}`);
  assert(swiftRow?.done === true, `Swift-created Boolean value was not readable by Core; received ${received}`);
}

function toPlainRows(rows) {
  return rows.map((row) => {
    if (!row || typeof row !== 'object' || !row.payload) return row;
    return Object.fromEntries(Object.entries(row.payload).map(([name, entry]) => [name, entry?.value]));
  });
}
...
async function verify() {
  ...
  try {
    ...
    console.log(JSON.stringify({ phase: 'verify', rows: rows.length, remotePath: config.remotePath }));
  } finally {
    await db.disconnect();
  }
}
...
async function verifySwiftBootstrap() {
  const { adapter, config, db } = createDatabase('core_interop_read_swift_bootstrap', 'swift-bootstrap');
  await db.init();
  try {
    await db.connect();
    const manifest = await readManifest(adapter, config.remotePath);
...
async function verifySwiftCompacted() {
  const { adapter, config, db } = createDatabase('core_interop_read_swift_snapshot', 'swift-bootstrap');
  await db.init();
  try {
    const changeFiles = await adapter.listFiles(`${config.remotePath}/changes`);

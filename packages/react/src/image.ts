...
import type { Interocitor, StoredImageMetadata } from '@interocitor/core';
 ...
 *
 * Reads `path` via `db.getImageBlobUrl()`, returns the generated `blob:` URL,
...
>(db: Interocitor<S>, path: string | null | undefined): UseImageResult {
  ...
  useEffect(() => {
    ...
    void db.getImageBlobUrl(path).then((image) => {
      if (cancelled) {
        image.revoke();
        return;
      }

...
test('putImage/getImage/getImageBlobUrl handles data URLs and revokable blob URLs', async ({ page }) => {
  const result = await page.evaluate(async () => {
    ...
    const engine = new Interocitor({ appName: 'ImageTest', dbName: 'image-test', schema, remotePath: '/mesh', keySource: new PortablePassphraseKeySource({ portableKey: passphrase }), localStore: new MemoryLocalStore() });
    await engine.init();

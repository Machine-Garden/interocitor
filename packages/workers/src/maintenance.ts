...
export async function runMaintenance(
  ...
  const rows = prefix
    ? await db.all<MaintenanceSweepRow>(
        "SELECT remote_root, last_operation_at, deleted_at FROM mesh_paths WHERE prefix = ?",
        prefix,
      )
    : await db.all<MaintenanceSweepRow>(
        "SELECT prefix, remote_root, last_operation_at, deleted_at FROM mesh_paths",
      );
  ...
  for (const row of candidates) {
    ...
    await db.batch([
      ...
      db
        ...
        .bind(threshold, threshold, threshold, targetPrefix, remoteRoot),
    ]);
    deleted += 1;
  }

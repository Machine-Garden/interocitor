...
function wrapSchemaError(error: unknown): never {
  ...
  if (/no such table: (files|folders)/i.test(message)) {
    throw new Error(
      "Interocitor D1 schema missing. Seed your database with @interocitor/workers/schema.sql before serving requests. Original error: " +
        message,
    );
  }
...
function resolveRuntimeConfig<Env>(
  ...
  return {
    ...
    maxGenericFileBytes: parsePositiveInt(
      runtime?.maxGenericFileBytes?.(env),
      DEFAULT_GENERIC_FILE_BYTES,
    ),
    maxStoredFileBytes: parsePositiveInt(
      runtime?.maxStoredFileBytes?.(env),
      DEFAULT_STORED_FILE_BYTES,
    ),
    maxMeshStoredBytes: parsePositiveInt(
      runtime?.maxMeshStoredBytes?.(env),
      DEFAULT_MESH_STORED_BYTES,
    ),
    authorizeFileUpload:
      runtime?.authorizeFileUpload as InterocitorRuntimeOptions<unknown>["authorizeFileUpload"],
    storageOperationAudit:
      runtime?.storageOperationAudit as InterocitorRuntimeOptions<unknown>["storageOperationAudit"],
...
function resolveDatabase<Env>(env: Env, dbGetter: (env: Env) => D1Database): DatabaseAdapter {
...
function applyConfiguredCors<Env>(
  response: Response,
  request: Request,
  env: Env,
  ...
  if (cors) {
    const origins =
      typeof cors.allowedOrigins === "function" ? cors.allowedOrigins(env) : cors.allowedOrigins;
...
async function emitAudit<Env>(
  runtime: ResolvedRuntimeConfig,
  env: Env,
  event: Omit<WorkerAuditEvent, "event" | "at">,
): Promise<void> {
...
async function handleWriteFile<Env>(
  ...
  const shouldBroadcast = ![PATH_TYPE.DEVICE_HEARTBEAT, PATH_TYPE.MANIFEST_SNAPSHOT].includes(
    pathType as never,
  );
  const notify = async (status: number, wrote = true): Promise<Response> => {
    await emitAudit(runtime, env, {
      ...
      pathType,
      status,
      outcome: status >= 200 && status < 300 ? "ok" : "rejected",
...
async function handleDelete<Env>(
  ...
  if (deleted) {
    broadcast(
      ...
      ctx,
      prefix,
      { type: "invalidation", op: "delete", path: normalizePath(path), ts: Date.now() },
...
async function handleStoredFileMetadata<Env>(
  ...
  const row = await db.first<StoredFileRow>(
    "SELECT * FROM stored_files WHERE prefix=?1 AND path=?2 LIMIT 1",
    prefix,
    normalized,
  );
  if (!row) {
...
async function handleGetStoredFile<Env>(
  db: DatabaseAdapter,
  ...
  const normalized = normalizePath(path);
  const row = await db.first<StoredFileRow>(
    "SELECT *, r2_key AS body_key FROM stored_files WHERE prefix=?1 AND path=?2 LIMIT 1",
    prefix,
    normalized,
  );
  if (!row?.body_key) {
  ...
  await db.run(
    "UPDATE stored_files SET last_accessed_at=?3, use_count=use_count+1 WHERE prefix=?1 AND path=?2",
    prefix,
    normalized,
    now,
  );
...
async function handlePutStoredFile<Env>(
  ...
  const bytes = await readBytes(request);
  if (!bytes) return jsonResponse({ error: "Invalid request body" }, 400);
  if (bytes.byteLength > runtime.maxStoredFileBytes)
    return jsonResponse({ error: "Payload too large", limit: runtime.maxStoredFileBytes }, 413);
  const normalized = normalizePath(path);
...
async function handleDeleteStoredFile<Env>(
  db: DatabaseAdapter,
  store: FileBodyStore | undefined,
  prefix: string,
  path: string,
...
async function handleSystemOperation<Env>(
  ...
  try {
    ...
    const integrityRequest = request.clone();
  ...
  return runMeshMiddleware(
    ...
    async () => {
      ...
      if (
        ...
        url.pathname.split("/").filter(Boolean)[2] === "health"
      ) {
  ...
  return runMeshMiddleware(
    ...
    async () => {
      ...
      if (op === "file") {
        ...
        if (method === "DELETE")
          return handleDelete(db, storageKey, path, ctx, runtime, relay, env, requestId(request));
      }
      ...
      if (op === "stored-file-metadata" && method === "POST") {
        const body = await readJsonBody(request);
        return handleStoredFileMetadata(
          db,
          storageKey,
          String(body?.path || "/"),
          request,
      ...
      if (op === "ensure-folder" && method === "POST") {
        await opListChildren(
          db.raw,
          storageKey,
          normalizePath(String((await readJsonBody(request))?.path || "/")),
        ).catch(() => null);
        return emptyResponse(204);
      }
...
async function handleRecoveryRequest<Env>(
  ...
  if (!RECOVERY_LOCATOR_RE.test(locator))
    return jsonResponse({ error: "Invalid recovery locator" }, 400);
  ...
  if (request.method === "PUT") {
    const bytes = await readBytes(request);
    if (!bytes) return jsonResponse({ error: "Invalid request body" }, 400);
    if (bytes.byteLength > runtime.maxControlBytes)
      return jsonResponse({ error: "Payload too large", limit: runtime.maxControlBytes }, 413);
    const result = await opPutImmutable(
      db.raw,
    ...
    await emitAudit(runtime, env, {
      ...
      path,
      status,
      outcome: result.wrote ? "ok" : "rejected",
      bytes: bytes.byteLength,
      requestId: requestId(request),
    });
    return emptyResponse(status);
...
export function createInterocitorMount<Env = unknown>(
  ...
  async function fetch(request: Request, env: Env, ctx: ExecutionContextLike): Promise<Response> {
    ...
    return interocitorWorker.fetch(
...
export function createInterocitorSystemHandler<Env = unknown>(
  ...
  async function fetch(request: Request, env: Env, _ctx: ExecutionContextLike): Promise<Response> {
    ...
    if (request.method.toUpperCase() === "OPTIONS")
      return applyConfiguredCors(preflightResponse(), request, env, options.cors);
...
export function withInterocitor<Env = unknown>(
  ...
  const { mountPrefix, cors, db, relay, files, runtime } = options;
  const interocitor = createInterocitorMount<Env>({ mountPrefix, cors, db, relay, files, runtime });
...
const interocitorWorker = {
  async fetch<Env = unknown>(
    ...
    if (url.pathname === "/" || url.pathname === "/health") {
      return applyConfiguredCors(
        withCors(new Response("interocitor cloudflare worker\n", { status: 200 })),
        request,
        env,
        cors,
      );
    ...
    if (url.pathname.startsWith(`${NOTIFY_PREFIX}/`)) {
      const prefix = decodeURIComponent(
        url.pathname.slice(`${NOTIFY_PREFIX}/`.length).split("/")[0] || "",
      );
      return applyConfiguredCors(
        await handleWsUpgrade(request, env, runtime, ctx, prefix, relayGetter),
        request,
        env,
        cors,
      );
    ...
    if (url.pathname.startsWith(`${IO_PREFIX}/`)) {
      return applyConfiguredCors(
        await handleIoRequest(request, env, runtime, ctx, url, db, relayGetter, filesGetter),
        request,
        env,
        cors,
      );
    ...
    if (url.pathname.startsWith(`${RECOVERY_PREFIX}/`)) {
      const locator = decodeURIComponent(
        url.pathname.slice(`${RECOVERY_PREFIX}/`.length).split("/")[0] || "",
      );
      return applyConfiguredCors(
        await handleRecoveryRequest(request, env, runtime, resolveDatabase(env, db), locator),
        request,
        env,
        cors,
      );

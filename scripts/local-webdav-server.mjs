...
const webDavBackend = options.mode === 'file' ? makeFileBackend(options.dataRoot) : makeMemoryBackend();
...
async function handleWebDav(req, res) {
  ...
  if (method === 'PROPFIND') {
    ...
    if (depth === '0') {
      ...
      const href = `${WEBDAV_PREFIX}/${encodePath(path)}`;
      const body = makePropfindResponse([
        makeResponseNode({ href, size: file.data.length, modifiedIso: file.modifiedTime, etag: file.etag, isCollection: false }),
      ]);
      res.writeHead(207, { 'Content-Type': 'application/xml; charset=utf-8' });
      res.end(body);
      return;
